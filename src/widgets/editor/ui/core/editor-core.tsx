...
/**
 * Renders the core editor interactions for localized textarea content.
 */
export const EditorCore = ({
  ...
  publishPendingLabel = 'Publishing...',
}: EditorCoreProps) => {
  ...
  /**
   * Resets the mobile pane state when the layout returns to desktop mode.
   */
  useEffect(() => {
    if (!isMobileLayout) {
  ...
  /**
   * Schedules a draft save after the autosave delay when eligible.
   */
  ...
  /**
   * Updates the localized title value.
   */
  ...
  /**
   * Updates the localized description value.
   */
  ...
  /**
   * Updates the localized content value.
   */
  ...
  /**
   * Converts pasted URLs into markdown link syntax when appropriate.
   */
  ...
  return (
    ...
    >
      <div className={metaStackClass}>
        ...
        {!hideTagSelector ? (
          <TagSelector
            ...
            emptyText={
              contentType === 'project' ? 'No tech stack options are available.' : undefined
            }
            onChange={setSelectedTags}
            onExpandedChange={() => {
              setLayoutShiftToken(previous => previous + 1);
            }}
            poolLabel={contentType === 'project' ? 'Tech stack selector' : undefined}
            poolTitle={contentType === 'project' ? 'Tech Stack' : undefined}
            selectLabel={contentType === 'project' ? 'Tech stack' : undefined}
            selectedTagSlugs={selectedTags}
          />
        ) : null}
      </div>

      <div aria-label="Language selection" className={localeTabListClass} role="tablist">
      ...
      {toastItems.length > 0 ? (
        <ToastViewport closeLabel="Close" items={toastItems} onClose={closeToast} />
      ) : null}
    </section>
  );
};
